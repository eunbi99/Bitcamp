...
public class BoardEx01 {
            ...
            if(userChoice == 1) {
                ...
                if(userList.isEmpty()) {
                  ...
                  //logIn 메소드 실행 후애
                    ...
                    //게시판의 메뉴를 불러오는 showMenu()메소드를 실행한다.
                    if(logIn != null) {
                        showMenu();
                    }
                }
    ...
    private static void showMenu() {
        while(true) {
            ...
            if(userChoice ==1) {
                ...
                //지금은 콘솔이기 때문에 별 차이가 없는 것 처럼 보이지만
                //나중에 웹 프로젝트에서는 로그아웃 혹은 로그인 정보가 만료된 이후에
    ...
    private static void printAllBoard() {
        ...
        if(boardList.isEmpty()) {
            ...
            SimpleDateFormat sdf = new SimpleDateFormat("yy.MM.dd");
                ...
                //Board 객체를 만들어서 그 객체가 boardList에 존재하는지 체크한다.
                Board b =new Board();
                b.setId(userChoice);
                
                //만약 userChoice가 0이 아니고 boardList의 contains(b) 결과가 false인 경우
    ...
    private static void printOneBoard(int id) {
      ...
      //파라미터로 받은 id값을 사용하여
        ...
        //b가 null일 경우, id가 잘못 되었다라는 의미이므로
        //경고 메시지만 출력
        ...
        System.out.println("==============================");
        //만약 댓글이 존재하지 않으면
        //"등록된 댓글이 존재하지 않습니다." 라는 메시지만 출력해주고
        //그 외에는 list의 Reply 객체의 print메소드를 실행시켜주자.
        if(list.isEmpty()) {
        ...
        //수정 삭제 뒤로가기를  출력해주고
        //그외에는 댓글달기와 뒤로가기만 출력해준다.
       //위의 옵션에 따라 해당하는 메소드를 실행해준다.
        int userChoice = 0;
        String message ;
        if(logIn.getId() == b.getWriterId()){
            ...
            //댓글달기 댓글삭제 뒤로가기 중 하나를 선택 할 수있게 한다.
            ...
            if(userChoice ==1) {
                ...
                if(list.isEmpty()) {
                   ...
                   while(!(replyId ==0 || list.contains(r))) {
                   ...
                   if(userInput != 0) {
                       ...
                       //먼저 댓글 작성자의 id와 로그인한 유저의 id비교를 위해
                       ...
                       if(logIn.getId()==r.getWriterId()) {
                           ...
                           System.out.println();
                           System.out.println("해당 댓글의 작성자만 댓글을 삭제하실수 있습니다.");
                           System.out.println();
                       }
                   }
                   printOneBoard(id);
    ...
    //게시글의 id값을 파라미터로 받아 해당 게시글의 댓글을 추가하는 메소드
    private static void addReply(int boardId) {
        //정보를 담을 Reply 객체 생성
        Reply r = new Reply();
        
        //id를 replyId 값을 주고 reply++
        r.setId(replyId++);
    ...
    private static void deleteBoard(int boardId) {
        //먼저 정말로 게시글을 삭제할것인지 물어본다.
        String message =new String("정말로 게시글을 삭제하시겠습니까? 삭제하려면 Y 혹은 y를 입력해주세요.");
        String yesNo = ScannerUtil.nextLine(scanner, message);
        
        if(yesNo.equalsIgnoreCase("Y")) {
            //현재 게시글을 게시글 리스트에서 삭제
            
            //임시로 객체 b를 만들어서 해당 객체에 boardId 값을 set한다.
            Board b = new Board();
            b.setId(boardId);
            
            //list의 remove메소드를 통해 b를 삭제한다.
            boardList.remove(b);
    ...
    //사용자가 넘겨준 id값으로 
    //replyList중에서 boardId가 일치하는 Reply 객체들을
    //어레이 리스트에 담아서 보내주는 selectReplyById() 메소드
    ...
    //사용자가 넘겨준 id값으로 일치하는 객체를 찾아 리턴해주는
    //retrieveBoard() 메소드
    ...
    private static void writeBoard() {
        ...
        //이게시글의 작성자 id는 당연히 로그인한 회원의 id가 되어야한다.
    ...
    //회원리스트중에서 중복된 아이디가 있는지 체크하는
    //validateId() 메소드
    private static boolean validateId(User u) {
        for(int i = 0; i< userList.size(); i++) {
            //for문을 돌면서 userList에 userName과 입력한 userName이 같은지 확인해서
            //만약 같다면 true를 리턴해라.
