package model;
/*MVC 패턴
 * MVC 패턴이란 클래스 분리를 할 때 사용 가능한 한가지의 개발 패턴으로서
 ...
 *      객체의 형태로 가공만 해주기 때문에 필드, 겟터/셋터, 그리고 java.lang.Object로 부터 상속 받은 메소드 중
 *      오버라이드할 메소드들만 적혀져 있다.
 *      모델의 경우 이름이 DTO 혹은 VO로 끝날 때가 많은데
 *      만약 해당 객체의 값이 수정도 되는 경우에는 DTO , 값을 읽어오는 용도 일때에는 VO라고 하지만
 *      사실 의미하는 바가 그렇게 큰 차이가 없으므로 둘 중 아무거나 적어도 되긴 한다.
 *      
 * V: Viewer
 *      뷰어는 화면에 출력을 담당하는 클래스가 된다.
 *      웹을 통해서 출력을 할 때에는 뷰어 클래스가 딱히 없을 수도 있는데
 *      왜냐하면 컨트롤러가 직접 웹한테 필요한 데이터를 전송하게 되면, 그러한 뷰어 클래스는 생략 가능하기 때문이다.
 *      만약 웹인데 뷰어 클래스를 사용하는 경우에는, 웹에서 출력할 내용을 뷰어가 모두 다 만들어서 보내주는 경우가 그러한 경우이다.
