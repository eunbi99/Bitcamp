...
public class TeacherController {
    //TeacherDTO 목록을 담당할 ArrayList필드 list
    ...
    //3.사용자가 입력한 TeacherDTO객체의 username과 password가 일치하는
    ...
    public ArrayList<TeacherDTO> selectByGroup(int group){
        ArrayList<TeacherDTO> temp = new ArrayList<>();
        for(TeacherDTO t : list) {
            if(t.getGroup() == group) {
                temp.add(t);
            }
    ...
    //사용자가 입력한 username이 중복인지 체크하는 validateUsername()
    ...
    //사용자가 입력한 group이 중복인지 체크하는 validateGroup()
    ...
    public TeacherDTO selectOne(int id) {
        for(TeacherDTO t : list) {
            if(t.getId() == id) {
                return t;
            }
