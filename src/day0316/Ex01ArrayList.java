 ...
 * 모든 메소드들은 처음부터 다시 만들어 주어야한다!  -> 매우 귀찮고 불편함.
 * 
 * 그래서 자바에는 기본적으로 여러 개의 같은 클래스 객체들을 다룰수 있게
 ...
 * 파라미터 혹은 리턴 타입에 인터페이스를 적어주고, 실제 리턴 되거나 파라미터로 들어올 때에는
 ...
 * 2.Set
 *      중복 입력이 불가능한 동적 할당이 구현된 배열이라고 생각하면 편하다.  
 * 3.Map
 *      맵은 배열과는 다르게 순서가 존재하지 않고, 대신 우리가 key를 사용하여
 *      우리가 저장한 객체를 꺼내오는 개념이다.
 * 물론, 리스트, 셋, 맵은 그 자체는 또 다른 인터페이스이고, 그리고 그 인터페이스를 상속받는
...
public class Ex01ArrayList {
    public static void main(String[] args) {
         ...
         * 
         *  Q1 : 그러면 템플릿 자리에 기본형 데이터 타입은 들어갈 수 있나요?
         *      A.못 들어갑니다.
         *  Q2 : 그러면 템플릿 자리에 배열은 들어갈 수 없나요?
         *      A.못 들어갑니다.
         *  Q3.정수를 모아둔 ArrayList가 필요한데 그럼 못 만드나요?
         *      A.아니요, 만들 수 있습니다.
         *      
         * 자바에서는 기본적으로 클래스로 만들어 놓은 기본형 데이터타입들이 있다.
         * 기본형 데이터타입을 클래스 형태로 "포장" 해 놓은 형태이기 때문에 우리가 "포장 클래스(Wrapper Class)"라고도 부른다.
         * 대표적으로 int를 포장한 Integer 클래스, double을 포장한 Double 클래스 등이 존재한다.
         * 따라서, 숙제로 해올 
         * 로또게임의 ArrayList버전 같은 경우 , 저 <> 자리에 Integer라고 적어주면 된다!
         * 
         * Student클래스의 ArrayList를 만들어보자
         */
        ...
        Student s2 = new Student(1,"A2",2,2,2);
        Student s22 = new Student(1,"A2",2,2,2);
        Student s3 = new Student(1,"A3",3,3,3);
        Student s33 = new Student(1,"A3",3,3,3);
       
        
        //2.어레이 리스트에 새로운 객체를 추가할 때에는
        // add(객체) 메소드를 실행하면 된다.
        ...
        //7.리스트안에 특정 객체가 존재하는지 확인하고 싶을 때에느
        ...
        //8.리스트안에 특정 객체의 가장 먼저 등장하는 인덱스가 궁금 할 때에는
        ...
        System.out.println("8. lastIndexOf(e)");
        list.add(s3);
        System.out.println("list.indexOd(s3) : " + list.indexOf(s3));
        System.out.println("list.lastIndexOd(s3) : " + list.lastIndexOf(s3));
        System.out.println();
        
        //10.리스트 안에 특정 객체를 삭제 할때에는
        ...
        list.remove(s33);
        System.out.println("list.indexOf(s3) : " + list.indexOf(s3));
        System.out.println("A. 해당 객체가 존재하지 않는 경우");
        System.out.println("list.size() : " + list.size());
        list.remove(s1);
        System.out.println("list.size() : " + list.size());
        System.out.println();
