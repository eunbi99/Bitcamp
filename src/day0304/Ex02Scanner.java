package day0304;
/*Scanner
 * Scanner클래스는 자바에서 다양한 입력을 처리해주는 클래스이다.
...
public class Ex02Scanner {
    public static void main(String[] args) {
         ...
         * 단 초기화시에는 반드시 어디서 입력을 읽어올지를 지정해와야 하기 때문에
         * 키보드 입력을 할 때에는 () 안에 System.in 이라고 적어주어야한다.
         * 
         */
        Scanner scanner = new Scanner(System.in);
        
        /* 1.사용자로 부터 int를 입력 받을 때에는
        ...
        /* 스캐너 객체를 키보드 입력을 위해 사용할 때에는 한 가지 주의할 점이 있는데
         * 만약 nextInt(), nextDouble() 등의 숫자, 실수를 입력하는 메소드를 실행하고 나서
         * nextLine() 메소드를 실행하게 되면
         * nextInt(), nextDouble()에서 사용자가 입력을 종료하기 위해 눌렀던 엔터키(공백문자 \n)이
         * 버퍼메모리에 남아있고 그 엔터키를 보고 nextLine()메소드는 "사용자가 아무런 글자 입력을 하지 않고 입력을 끝냈다"고 착각하는 버그 존재.
         * 
         * 해당 버그를 해결하기 위해서는
         * nextInt(), nextDouble() 등의 숫자, 실수를 입력하는 메소드 "이후에는" 반드시 
         * nextLine() 메소드를 한번 실행해서 버퍼메모리를 비워주고
         * 그 다음에 String입력을 nextLine()으로 받아주어야한다.
         * 
         * 단, 프로그램을 시작하자마다 String입력을 위해
         * nextLine()을 쓰는 경우, 버퍼메모리가 깨끗한 상태이기 때문에 
