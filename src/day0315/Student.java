package day0315;
/*구조체 (Struct)
 * 2세대 언어에서는 우리가 여러가지 데이터 타입을 섞어서
 * 필요한 한가지 데이터타입으로 만들어주던 구조체 라는 특수한 데이터 타입이 있었다.
 * 예를 들어, 우리가 학생의 구조체를 만든다고 한다면
 * 그안에, 
 * 번호, 이름, 국어 점수 , 수학 점수, 영어 점수 를 넣어서
 * 하나의 Student 이라는 데이터타입으로 만들어주게 되는 것이다.
 * 
 * 하지만 3세대 언어인 자바에서는 구조체가 존재하지않는다.
 * 그렇지만 우리가 구조체를 비스무리하게 만들어 줄 수는 있다.
 * 바로 아래가 Student라는 구조체를 비스무리하게 만드는 방법이다.
 * 
 * 전혀 길게 기억할 필요가 없는 자바에서 구조체만드는 방법
 *
 ...
 * 자바가 갖고있는 구조체보다 더 좋은 것이 바로 클래스이다.
 * 
 * 클래스란?
 * 자바가 제안하는 프로그램의 초소단위가 바로 클래스이다.
 * 클래스는 필드와 메소드로 이루어져 있다.
 * 필드 : 해당 클래스의 객체들이 저장할 정보들의 종류를 규정한것
 *      필드는 우리가 변수를 선언하는 것과 매우 유사하지만,
 *      필드의 초기화는 우리가 직접 하지 않고, 그 객체를 생성 할 때
 ...
 * 예시 : 우리가 만약 삼성전자 디자이너이고 다음 갤럭시를 설계하는것.
 *      다음 갤럭시에 들어갈 정보들 :시리얼 번호, 모델명 , 메모리 용량 같은 정보
 *      다음 갤럭시에 들어갈 기능들 : 사진 촬영하기(), 지문인식하기(), 삼성페이로 돈 내기()
 *      
 * 우리는 이렇게 대략적인 설계도만 만들고,
 * 그리고 메인 메소드나 다른 메소드가 필요하다면 이설계도로 객체를 직접 만들어서
 * 쓰게 되는 것이다.
 * 
 ...
 * 
 * 또한 학생 클래스의 객체라면
 * 총점 계산 , 평균 계산, 성적 예쁘게 츨력해주기의 기능이 필요하게 될 것이다.
 * 그렇다면, 우리는 그것을 Student클래스 내부에 만들어주고
 * 그 메소드의 실행은 ? 그 Student클래스 객체가 따로 해주면 되는 것이다.
 * 
 */
public class Student {
    //필요한 데이터의 종류를 여기에 적는다.
     ...
     public Student(int id, String name, int korean, int english , int math) {
         ...
         //따라서 아래의 코드는
         //파라미터 id에 파라미터 id의 현재 값을 할당해라 라는 의미가 된다.
         //id =id ;
         //하지만 우리가 원하던 것은
         //이 메소드를 실행시키는 객체의 필드 id에 파라미터 id의 현재값을 할당해라 이므로
         //다음과 같이 
         //객체의 필드 id를 특별하게 지정해야한다.
         this.id=id;
         this.name =name;
         this.korean = korean;
     ...
     public boolean equals(Object o) {
         ...
         if(o instanceof Student) {
             ...
             //실체화 되어있는 객체라고 한다.
             //또는 instance라고 부르기도한다.
             
             //같은 클래스의 객체이므로 typecasting을 통하여
             //우리가 임시적으로 Student클래스 객체로 만들어 줄 수 있다.
             Student s = (Student)o;
