 ...
 * 호출되는 메소드이다.
 * 객체가 처음 초기화 될때
 * 필드의 초기값이나 특정한 행동을 해야 할 경우 , 우리가 직접 만들어 줄 수 있다.
 * 생성자는, 우리가 특별히 클래스 안에 만들어 주지 않더라도
 * 자바가 기본적으로 제공된다.
 * 이러한 기본 생성자는 특별한 행동은 하지 않고
 * 기본형 데이터타입을 가진 필드는 0으로 
 * 참조형 데이터타입을 가진 필드는 null로 초기화해준다.
 * 
 * 우리가 직접 생성자를 만들어 줄 수도 있는데
 * 이때 주의해야할 점은
 * 우리가 파라미터가 있는 생성자만 만들어 줄 경우,
 * 더이상 파라미터가 없는 생성자는 호출 할 수 없다. (예시: Scanner)
 ...
 * 자식 클래스가 또 안만들어도
 * 그대로 쓸 수 있게 하기 위해 사용된다.
 * 
 * 우리가 직접 상속을 구현 할 수 도 있지만
 * 사실 우리가 만드는 모든 클래스들은 우리 모르게 상속을 받고있다.
 * 자바의 모든 클래스는 java.lang.Object라는 클래스를 상속받는다.
 * 
 ...
 * 비교를 해야하는지 모른다는 것이다.
 * 그래서, java.lang.Object안의 메소드들은 모두다
 * 매우 애매모호 하게 구현되어있다.
 * 따라서 , 우리 클래스가 해당 메소드들을 그대로 쓰기에는 문제가 있다.
 * 그럴 경우에는 상속받은 메소드를 자식 클래스가 재정의 해주어야 하는데
 * 
 * 이러한 메소드 재정의를 우리가 "오버라이드(Override)"라고 부른다.
 * 오버라이드를 할 때 주의할 점은 부모클래스가 메소드를 선언한 것과 똑같이 선언을 해주어야 한다는 것이다!
 * 
 * 오버라이드 예시 )
 * 1.equals
 * java.lang.Object는 equals 메소드를 다음과 같이 선언과 구현해 놓았다.
 ...
 * 
 * 다형성(polymorphism)
 * 다형성이란 "다"양한 "형"태를 가지는 "성"격을 다형성이라고 한다.
 * 자바에서 다형상이란, 파라미터나 리턴 타입에 부모클래스를 적어주면
 * 해당 부모클래스와 그 부모 클래스를 상속받는 모든 클래스가 그 자리에 대신 할 수 있는 성격을
 * 다형성이라고 한다.
 * 예를 들어서, 우리가 equals 메소드의 파라미터로 Object o 라고 적어주면
 * equals메소드를 사용 할 때 해당 메소드 파라미터는 Object클래스를 상속받는 모든 클래스가
 * 들어갈 수 있게 된다.
 */
